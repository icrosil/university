...
def _op_to_function_op(oper):
    ...
    def function_op(self, operand):
        ...
        def func(*args):
            """Result function."""
            return oper(self(*args), FuncOperationHandler(operand)(*args))
        return FuncOperationHandler(func)
    return function_op
...
for name, op in [(name, getattr(operator, name)) for name in dir(operator) if "__" in name]:
    try:
        ...
        setattr(FuncOperationHandler, name, _op_to_function_op(op))
        if name in RIGHT_OPERATORS:
            setattr(FuncOperationHandler, RIGHT_OPERATORS[name], _op_to_function_op(op))
